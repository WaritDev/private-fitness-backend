...
func ProvideUserController(uc *services.UserService) *UserController {
	return &UserController{UC: uc}
}

// GET /api/users/check-phone?phone=0812345678
// ตรวจสอบว่าเบอร์โทรศัพท์ซ้ำหรือไม่ (Q3S.1)
func (h *UserController) CheckPhoneNumber(c *fiber.Ctx) error {
...
func (h *UserController) CheckGmail(c *fiber.Ctx) error {
	gmail := c.Query("gmail")
	if gmail == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"status":      "Bad Request",
...
func (h *UserController) CheckUsername(c *fiber.Ctx) error {
